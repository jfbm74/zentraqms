...
class Organization(FullBaseModel):
    ...
    razon_social = models.CharField(
        _('razón social'),
        max_length=200,
        help_text=_('Razón social completa de la organización según documentos legales.')
    )
    ...
    nit = models.CharField(
        ...
        validators=[
            RegexValidator(
                regex=r'^\d{9,10}-?\d?$',
                message=_('NIT debe tener formato válido: 123456789-0 o 1234567890')
            )
        ],
        ...
        if self.nit and self.digito_verificacion:
            ...
            if str(digito_calculado) != self.digito_verificacion:
                raise ValidationError({
                    'digito_verificacion': _('El dígito de verificación no corresponde al NIT ingresado.')
                })
...
class Location(FullBaseModel):
    ...
    ciudad = models.CharField(
        _('ciudad'),
        max_length=50,
        help_text=_('Ciudad donde se encuentra la sede.')
    )
    ...
    email = models.EmailField(
        _('email'),
        blank=True,
        help_text=_('Email de contacto de la sede.')
    )
    ...
    def save(self, *args, **kwargs):
        ...
        # Si es la primera sede de la organización, marcarla como principal
        if not self.pk and not Location.objects.filter(organization=self.organization).exists():
...
class SectorTemplate(FullBaseModel):
    ...
    data_json = models.JSONField(
        ...
        help_text=_('Configuración predefinida en formato JSON para aplicar a la organización.'),
        default=dict
    )
    ...
    def aplicar_a_organizacion(self, organization, usuario=None):
        ...
        if organization.sector_economico != self.sector:
            raise ValidationError(
                _('Este template es para el sector {}, pero la organización es del sector {}.').format(
        ...
        try:
            with transaction.atomic():
                ...
                if 'indicadores' in self.data_json:
                    resultado['elementos_creados']['indicadores'] = self._aplicar_indicadores(
                        organization, self.data_json['indicadores'], usuario
                    )
                
                # Aplicar configuración de documentos
                if 'documentos' in self.data_json:
                    resultado['elementos_creados']['documentos'] = self._aplicar_documentos(
                ...
                self.save(update_fields=['aplicaciones_exitosas', 'fecha_ultima_aplicacion', 'updated_by', 'updated_at'])
...
class AuditLog(FullBaseModel):
    ...
    def log_change(cls, instance, action, user=None, old_values=None, new_values=None, 
                   changed_fields=None, request=None, reason=None):
        """
        ...
        if request:
            ...
            # Get user agent
            user_agent = request.META.get('HTTP_USER_AGENT', '')[:1000]  # Truncate to avoid overflow
    ...
    def can_rollback(cls, instance, target_log_id):
        ...
        try:
            ...
            if newer_logs.exists():
                return False, _('No se puede hacer rollback porque el registro ha sido eliminado.')
    ...
    def perform_rollback(self, user=None, request=None, reason=None):
        ...
        try:
            ...
            if self.table_name == 'organization_organization':
                ...
                return False, f'Rollback not supported for table {self.table_name}', None
