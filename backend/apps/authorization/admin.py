...
class PermissionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información Básica", {"fields": ("name", "code", "description")}),
        ("Detalles del Permiso", {"fields": ("resource", "action", "is_active")}),
        (
            "Auditoría",
...
class RoleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Información Básica", {"fields": ("name", "code", "description")}),
        ("Configuración", {"fields": ("is_system", "is_active")}),
        (
            "Auditoría",
            {"fields": ("id", "created_at", "updated_at"), "classes": ("collapse",)},
        ),
    ...
    def users_count(self, obj):
        ...
        count = (
            UserRole.objects.filter(role=obj, is_active=True, user__is_active=True)
            .exclude(expires_at__lt=timezone.now())
            .count()
        )
        url = (
            reverse("admin:authorization_userrole_changelist")
...
class UserRoleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Asignación", {"fields": ("user", "role")}),
        ("Estado", {"fields": ("is_active", "expires_at")}),
        (
            "Auditoría",
            {"fields": ("id", "assigned_at", "assigned_by"), "classes": ("collapse",)},
        ),
    )
...
class ExtendedUserAdmin(BaseUserAdmin):
    ...
    def display_roles(self, obj):
        ...
        if roles:
            return format_html(
                "<ul>{}</ul>", "".join([f"<li>{role}</li>" for role in roles])
            )
    ...
    def display_permissions(self, obj):
        ...
        if obj.is_superuser:
            return format_html(
                '<span style="color: red;">✓ Superusuario (todos los permisos)</span>'
            )
        ...
        if permissions:
            ...
            html = "<ul>{}</ul>".format(
                "".join([f"<li><code>{perm}</code></li>" for perm in perm_list])
            )
