...
class RoleViewSet(PermissionRequiredMixin, viewsets.ModelViewSet):
    ...
    def users(self, request, pk=None):
        ...
        user_roles = UserRole.objects.filter(
            role=role, is_active=True, user__is_active=True
        ).select_related("user")
    ...
    def assign_role(self, request):
        ...
        try:
            ...
            user_role = user.add_role(
                role_code=role_code, assigned_by=request.user, expires_at=expires_at
            )
...
class UserPermissionsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def check_permissions(self, request):
        ...
        return Response({"user_id": request.user.id, "permissions": results})
