            ...
            for perm_id in permission_ids:
                try:
                    ...
                    RolePermission.objects.create(
                        role=role, permission=permission, granted_by=user
                    )
        ...
        if permission_ids is not None:
            ...
            for perm_id in permission_ids:
                try:
                    permission = Permission.objects.get(id=perm_id, is_active=True)
                    RolePermission.objects.create(
                        role=instance, permission=permission, granted_by=user
                    )
                except Permission.DoesNotExist:
                    continue
...
class RoleListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Role
        fields = ["id", "name", "code", "is_system", "is_active", "total_permissions"]
    ...
    def validate(self, data):
        ...
        if role_id:
            try:
                ...
                raise serializers.ValidationError({"role_id": "El rol no existe"})
