...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("✨ Usuarios de prueba RBAC creados exitosamente")
        )
        ...
        for user_data in test_users_data:
            ...
            try:
                ...
                user, created = User.objects.update_or_create(
                    ...
                    defaults={**user_data, "is_active": True, "is_verified": True},
                )
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'❌ Error creando usuario {user_data["email"]}: {e}'
                    )
                )
        ...
        for user in test_users:
            if hasattr(user, "_role_code"):
                try:
                    ...
                    self.stdout.write(
                        f"  ✓ Rol {user._role_code} asignado a: {user.email}"
                    )
                except ValueError as e:
                    self.stdout.write(
                        self.style.ERROR(
                            f"❌ Error asignando rol {user._role_code} a {user.email}: {e}"
                        )
                    )
    ...
    def show_user_summary(self):
        ...
        users_with_roles = (
            User.objects.filter(user_roles__is_active=True)
            .distinct()
            .select_related()
