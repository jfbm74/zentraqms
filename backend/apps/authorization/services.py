...
class PermissionService:
    ...
    def evaluate_permission(cls, user: User, permission_code: str) -> bool:
        ...
        if cached_result is not None:
            logger.debug(f"Permission {permission_code} for user {user.id} found in cache: {cached_result}")
        ...
        logger.debug(f"Permission {permission_code} for user {user.id} evaluated: {result}")
    ...
    @classmethod
    def check_multiple_permissions(cls, user: User, permission_codes: List[str], 
                                 require_all: bool = False) -> Dict[str, bool]:
        """
    ...
    @classmethod
    def can_assign_role(cls, assigner: User, target_user: User, role_code: str) -> Tuple[bool, str]:
        """
        ...
        assigner_level = max(
            role_hierarchy.get(role, 0) for role in assigner_roles
        ) if assigner_roles else 0
    ...
    def invalidate_user_cache(cls, user_id: str):
        ...
        try:
            ...
            common_permissions = [
                ...
                'dashboard.view', 'dashboard.export', '*.all'
            ]
            ...
            logger.info(f"Cache keys deleted for user {user_id}: {len(keys_to_delete)} keys")
    ...
    @classmethod
    def can_assign_role_to_user(cls, assigner: User, target_user: User, role_code: str) -> bool:
        """
    ...
    @classmethod
    def audit_permission_check(cls, user: User, permission_code: str, 
                             granted: bool, context: Optional[Dict] = None):
        """
