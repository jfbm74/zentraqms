...
class RBACMiddlewareTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="test@zentraqms.test", first_name="Test", last_name="User"
        )
...
class PermissionCacheMiddlewareTests(TestCase):
    ...
    def setUp(self):
        self.factory = RequestFactory()
        self.middleware = PermissionCacheMiddleware(
            lambda request: JsonResponse({"success": True})
        )
    ...
    def test_middleware_ignores_non_permission_urls(self):
        ...
        with patch(
            "apps.authorization.services.PermissionService.clear_user_cache"
        ) as mock_clear_cache:
            response = self.middleware(request)
...
class PermissionDecoratorsTests(TestCase):
    ...
    def setUp(self):
        ...
        self.role = Role.objects.create(name="Document Creator", code="doc_creator")
...
class PermissionServiceTests(TestCase):
    ...
    def setUp(self):
        ...
        self.specific_perm = Permission.objects.create(
            name="Crear Documentos Específicos", code="documents.create"
        )

        self.wildcard_perm = Permission.objects.create(
            name="Todos los Documentos", code="documents.*"
        )

        self.super_perm = Permission.objects.create(name="Super Admin", code="*.all")

        # Crear roles
        self.specific_role = Role.objects.create(
            name="Document Creator", code="doc_creator"
        )

        self.wildcard_role = Role.objects.create(
            name="Document Manager", code="doc_manager"
        )

        # Asignar permisos
        RolePermission.objects.create(
            role=self.specific_role, permission=self.specific_perm
        )

        RolePermission.objects.create(
            role=self.wildcard_role, permission=self.wildcard_perm
        )
    ...
    def test_can_assign_role_to_user(self):
        ...
        admin_perm = Permission.objects.create(
            name="Asignar Roles", code="users.assign_roles"
        )

        admin_role = Role.objects.create(name="Admin", code="admin")
...
class AuthenticationEndpointsTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.role = Role.objects.create(name="Document Manager", code="doc_manager")
...
class RBACIntegrationTests(APITestCase):
    ...
    def setUp(self):
        ...
        # Crear roles
        self.admin_role = Role.objects.create(name="Administrador", code="admin")

        self.coordinator_role = Role.objects.create(
            name="Coordinador", code="coordinator"
        )

        self.auditor_role = Role.objects.create(name="Auditor", code="auditor")

        # Asignar permisos a roles
        # Admin: acceso completo
        RolePermission.objects.create(
            role=self.admin_role, permission=self.permissions["*.all"]
        )

        # Coordinador: usuarios y reportes
        RolePermission.objects.create(
            role=self.coordinator_role, permission=self.permissions["users.create"]
        )
        RolePermission.objects.create(
            role=self.coordinator_role, permission=self.permissions["users.read"]
        )
        RolePermission.objects.create(
            role=self.coordinator_role, permission=self.permissions["reports.create"]
        )

        # Auditor: solo auditorías
        RolePermission.objects.create(
            role=self.auditor_role, permission=self.permissions["audits.create"]
        )
        RolePermission.objects.create(
            role=self.auditor_role, permission=self.permissions["audits.read"]
        )
    ...
    def test_role_expiration_functionality(self):
        ...
        # Crear rol temporal
        temp_role = Role.objects.create(name="Temporal", code="temp")

        RolePermission.objects.create(
            role=temp_role, permission=self.permissions["users.read"]
        )

        # Asignar rol con expiración
        past_date = timezone.now() - timedelta(days=1)
        UserRole.objects.create(
            user=self.auditor_user, role=temp_role, expires_at=past_date
        )
    ...
    def test_multiple_roles_combination(self):
        ...
        UserRole.objects.create(user=self.auditor_user, role=self.coordinator_role)
        ...
        self.assertTrue(
            self.auditor_user.has_perm("reports.create")
        )  # Del rol coordinador
    ...
    def test_cache_invalidation_on_role_change(self):
        ...
        test_user = User.objects.create_user(
            email="cache_test@zentraqms.test", first_name="Cache", last_name="Test"
        )
        ...
        self.assertTrue(
            cache_clear_works, "El método clear_user_cache debe funcionar sin errores"
        )
